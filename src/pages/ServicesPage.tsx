    ...
    subServices: [
      {
        ...
        columns: {
          applications: ['Residential', 'Commercial', 'Hospitality', 'Institutional', 'Industrial'],
          keyBenefits: [
      ...
      },
      {
        ...
        columns: {
          applications: ['Commercial buildings', 'District cooling networks', 'Industrial facilities', 'Healthcare campuses', 'Mixed-use developments'],
          keyBenefits: ['Improved chiller efficiency', 'Reduced pumping energy', 'Lower operational costs', 'Extended equipment lifespan'],
          approach: ['System performance assessment', 'Data collection and analysis', 'Optimization strategy development', 'Implementation support', 'Performance verification'],
        },
      },
      {
        ...
        columns: {
          applications: ['New construction projects', 'Major renovations', 'Design-build projects', 'Government and institutional facilities', 'High-rise developments'],
          keyBenefits: ['Risk mitigation', 'Code compliance assurance', 'Design optimization', 'Cost avoidance'],
          approach: ['Design document review', 'Standards compliance check', 'Performance analysis', 'Recommendations report', 'Follow-up consultation'],
        },
      },
      {
        ...
        columns: {
          applications: ['Office buildings', 'Healthcare facilities', 'Educational institutions', 'Hospitality venues', 'Residential complexes'],
          keyBenefits: ['Improved occupant health', 'Enhanced productivity', 'Regulatory compliance', 'Reduced absenteeism'],
          approach: ['Environmental monitoring', 'Data analysis and benchmarking', 'Root cause identification', 'Improvement recommendations', 'Post-implementation verification'],
        },
      },
      {
        ...
        columns: {
          applications: ['New buildings', 'Building retrofits', 'System replacements', 'Capacity planning', 'Energy optimization studies'],
          keyBenefits: ['Right-sized equipment', 'Reduced capital costs', 'Lower energy consumption', 'Improved system performance'],
          approach: ['Building envelope analysis', 'Load simulation modeling', 'Peak and part-load analysis', 'System sizing recommendations', 'Energy impact assessment'],
        },
      },
      {
        ...
        columns: {
          applications: ['District cooling plants', 'Commercial complexes', 'Industrial cooling systems', 'Data centers', 'Hospital campuses'],
          keyBenefits: ['Reduced energy consumption', 'Improved plant efficiency', 'Lower maintenance costs', 'Enhanced system reliability'],
          approach: ['Plant performance audit', 'Hydraulic analysis', 'Control strategy optimization', 'Implementation and tuning', 'Ongoing monitoring and reporting'],
        },
      },
      {
        ...
        columns: {
          applications: ['Multi-tenant buildings', 'Mixed-use developments', 'Industrial complexes', 'Government facilities', 'Retail portfolios'],
          keyBenefits: ['Transparent cost allocation', 'Enhanced energy visibility', 'Regulatory compliance', 'Data-driven decisions'],
          approach: ['Metering strategy design', 'Hardware specification', 'Installation oversight', 'Data platform integration', 'Reporting and analytics setup'],
        },
      },
    ],
  },
...
export const ServicesPage = (): JSX.Element => {
  const [openSubId, setOpenSubId] = useState<string>('energy-efficiency-retrofit');
    ...
    <div className="services-page">
      ...
      <div className="services-body">
        {services.map((service) => (
          <section key={service.id} className="services-section" aria-labelledby={`service-${service.id}`}>
            <div className="services-section-inner">
              <header className="services-section-header">
                <div className="services-section-icon-wrap">
                  <img src={service.icon} alt="" aria-hidden="true" className="services-section-icon" />
                </div>
                <div className="services-section-header-text">
                  <h2 id={`service-${service.id}`} className="services-section-title">
                    {service.title.split('\n').map((line, i) => (
                      <span key={i}>
                        {line}
                  ...
                  {service.subtitle && <p className="services-section-subtitle">{service.subtitle}</p>}
                </div>
              </header>
              ...
              {service.subServices && service.subServices.length > 0 && (
                <div className="services-sub-list">
                  {service.subServices.map((sub) => {
                    ...
                    return (
                      <div key={sub.id} className={isOpen ? 'services-sub-item is-open' : 'services-sub-item'}>
                        <button
                          type="button"
                        ...
                        >
                          ...
                          <span className="services-sub-trigger-title">{sub.title}</span>
                        </button>
                          ...
                          {isOpen && (
                            ...
                            >
                              <div className="services-sub-content-inner">
                                <p className="services-sub-desc">{sub.description}</p>
